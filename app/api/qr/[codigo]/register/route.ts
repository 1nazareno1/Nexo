...
export async function POST(request: Request, { params }: { params?: Promise<{ codigo?: string }> } = {}) {
  try {
    ...
    const { nombre, especie, raza, color, telefono, imagen_url, observaciones } = body as Record<string, any>;
    ...
    if (!updatedQR) {
      return NextResponse.json({ error: "No se pudo actualizar el QR." }, { status: 500 });
    }
